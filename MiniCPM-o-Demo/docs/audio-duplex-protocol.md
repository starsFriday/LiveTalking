...
音频双工使用当前 Realtime API：

```text
wss://host/v1/realtime?mode=audio
```

客户端持续发送 16 kHz 单声道 float32 PCM 音频，base64 编码；服务端返回 24 kHz 单声道
float32 PCM 音频，base64 编码。会话总时长上限为 600 秒。
...
音频模式不需要携带视频帧。`force_listen` 也可以放在 `input.hints.force_listen`。
...
文本和音频是独立 delta，不保证一一对应。音频双工不使用 `response.done` 表示每轮结束；
